  ...
  return (
    <article className="mx-auto max-w-7xl px-4 py-8 md:px-6">
      ...
      <div className="grid gap-10 lg:grid-cols-[minmax(0,780px)_320px] lg:items-start">
        <main>
          <Link href={`/${article.category.slug}`} className="text-xs font-black uppercase tracking-[0.22em] text-accent">
            {article.category.name}
          </Link>
          <h1 className="mt-3 font-serif text-5xl font-black leading-[0.96] md:text-7xl">{article.title}</h1>
          <p className="mt-5 max-w-3xl text-xl leading-8 text-muted-foreground">{article.excerpt}</p>

          <div className="mt-6 flex flex-wrap items-center gap-3 border-y border-border py-3 text-sm font-semibold text-muted-foreground">
          ...
          <AdPlaceholder label="Formato nativo entre cabeçalho e texto" className="mt-8" />

          <div className="mt-8 border-t border-foreground/20 pt-5 text-lg">
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {article.tags.map((tag) => (
              <Link
                key={tag.slug}
                href={`/tag/${tag.slug}`}
                className="border border-border bg-surface px-3 py-2 text-sm font-bold hover:border-accent hover:text-accent"
              >
                #{tag.name}
              </Link>
            ))}
          ...
          <section className="mt-12">
            <h2 className="font-serif text-3xl font-black">Comentários</h2>
            <p className="mt-2 text-sm text-muted-foreground">
              O espaço é moderado para preservar debate público, civilidade e relevância.
            </p>
        ...
        <aside className="space-y-5 lg:sticky lg:top-24">
          <AdPlaceholder label="Formato lateral da matéria" />
          <section className="border border-border bg-surface p-5">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-accent">Arquivo relacionado</p>
            <h2 className="mt-1 font-serif text-2xl font-black">Para seguir o fio</h2>
            <div className="mt-4 space-y-4">
              ...
              {related.length === 0 ? (
                <p className="text-sm leading-6 text-muted-foreground">
                  Ainda não há matérias relacionadas publicadas.
                </p>
              ) : null}
            </div>
          </section>
      ...
      {related.length > 0 ? (
        <section className="mt-12">
          <h2 className="font-serif text-3xl font-black">Continue lendo</h2>
